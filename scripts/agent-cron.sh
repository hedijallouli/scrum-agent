...
generate_cycle_summary() {
  ...
  local CYCLE_TIME CYCLE_DATE CYCLE_STAMP
  CYCLE_TIME=$(date -u '+%H:%M')
  CYCLE_DATE=$(date -u '+%Y-%m-%d')
  CYCLE_STAMP=$(date -u '+%Y%m%d-%H%M')

  # ── Log file destination ──────────────────────────────────────────────────
  local LOG_WEB_DIR="/var/www/cdo-logs"
  local LOG_WEB_URL="${PROJECT_DEPLOY_URL:-http://49.13.225.201:3003}/logs"
  mkdir -p "$LOG_WEB_DIR"

  # Purge logs older than 7 days
  find "$LOG_WEB_DIR" -name "*.md" -mtime +7 -delete 2>/dev/null || true
  ...
  # Collect full agent logs from the last 20 min
  local LOG_SNIPPETS=""
  local MD_AGENT_SECTIONS=""
  local cutoff
  ...
  for lf in ${LOG_DIR}/${PROJECT_KEY}-*-*.log; do
    ...
    # Full log content for .md file
    MD_AGENT_SECTIONS+=$'\n'"## ${fname}"$'\n'"
\`\`\`"$'\n'"$(cat "$lf" 2>/dev/null || echo '(empty)')"$'\n'"\`\`\`"$'\n'
  done

  # ── Write full .md log file ───────────────────────────────────────────────
  local MD_FILE="${LOG_WEB_DIR}/${CYCLE_STAMP}.md"
  local MD_URL="${LOG_WEB_URL}/${CYCLE_STAMP}.md"
  cat > "$MD_FILE" << MDEOF
# ${PROJECT_KEY} Cron Cycle — ${CYCLE_DATE} ${CYCLE_TIME} UTC

**Failures:** ${FAILURES}/${#ALL_PIDS[@]}
**Stuck tickets:** ${STUCK_TICKETS:-none}
**Dispatched this cycle:**
${LOG_SNIPPETS:-_No agents ran this cycle._}

---
${MD_AGENT_SECTIONS:-_No agent logs available._}

---
_Generated by Omar · ${PROJECT_KEY} agent system_
MDEOF

  log_info "Cycle log written: ${MD_FILE}"

  # ── Generate short Mattermost summary ────────────────────────────────────
  log_info "Generating Omar cycle summary (${FAILURES} failures, stuck: ${STUCK_TICKETS:-None})..."

  local SUMMARY
  SUMMARY=$(claude -p --model haiku --max-turns 1 "Tu es Omar, le watchdog ops de ${PROJECT_KEY}. Rédige un résumé Mattermost COURT (max 3 lignes) du cycle de cron qui vient de se terminer.
...
Règles: sois factuel, mentionne les tickets spécifiques si bloqués, pas de remplissage générique. Max 2 emojis. Utilise **gras** pour les tickets. Ne mets PAS le lien dans le résumé (il sera ajouté automatiquement)." 2>&1) || true

  local LINK_LINE="[Voir le log complet](${MD_URL})"

  if [[ -n "$SUMMARY" && "$SUMMARY" != *"Not logged"* && ${#SUMMARY} -gt 10 ]]; then
    slack_notify "omar" "${SUMMARY}"$'\n\n'"${LINK_LINE}" "pipeline"
    log_info "Omar cycle summary posted"
  else
    slack_notify "omar" "Cycle ${CYCLE_TIME} UTC — ${FAILURES}/${#ALL_PIDS[@]} échecs. Bloqués: ${STUCK_TICKETS:-aucun}."$'\n\n'"${LINK_LINE}" "pipeline"
